...
Captures 92.6% of buy-and-hold upside with a better Sharpe and tighter drawdown. 128 of 136 tickers finished positive. Turnover of 0.01 means the agent holds rather than churning for cost.
...
IT underperformance was broad-based (0/10) and is a known soft spot; see the limitations note on uniform conviction.
...
- **Single-stock episodes.** Each environment instance trades one ticker in isolation. The "portfolio" is stitched together at evaluation, so the agent never learns cross-sectional allocation, correlation, or capital-constraint trade-offs.
